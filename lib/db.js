...
export function insertContent(contentTitle, contentDescription) {
  //   const db = openDB();
  const stmt = db.prepare("INSERT INTO contents (contentTitle, contentDescription) VALUES (?, ?)");
  stmt.run(contentTitle, contentDescription);
}

export function updateContent(id, contentTitle, contentDescriptioname) {
// export function updateContent(contentTitle, contentDescription) {
  //   const db = openDB();
  const stmt = db.prepare("UPDATE contents SET contentTitle = ?, contentDescription = ? WHERE id = ?");
  stmt.run(id, contentTitle, contentDescriptioname);
}
